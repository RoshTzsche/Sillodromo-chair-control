...
HEARTBEAT/CALIBRATED/MOUTH_OPEN); stderr: estado y diagnóstico. Ctrl+C o ESC terminan.
No envía serial ni invoca Alexa: integrar mediante GestureController.emit.
El consumidor de movimiento necesita watchdog propio y confirmación física
de parada antes de habilitar interacción. STOP aquí es una petición, no
telemetría. Este puente no garantiza latencia de control ni sustituye un
paro físico. No conectar directamente a motores sin verificar latencia,
neutro y protocolo del controlador — exactamente las mismas condiciones
que ya exigía openface.py; sólo cambia de dónde vienen los números.

QUÉ CAMBIA respecto a openface.py:
  - No hay subprocess ni CSV: FaceLandmarker corre in-process (Tasks API,
    VIDEO mode), igual que en tracker.py de RoshTzsche/multimodal-tracker.
  - No existe un equivalente directo a gaze_angle_x/y de OpenFace (ese es
    un modelo 3D de mirada entrenado). En su lugar se usa pose de cabeza
    (yaw/pitch) vía solvePnP con 6 puntos — es lo que openface.py ya
    soportaba como --signal head, y suele ser más robusto para controlar
    una silla que sostener la mirada de reojo.
  - Cejas y parpadeo son métricas geométricas nuevas (no AUs de OpenFace),
    con escalas propias. Los defaults de --brow/--blink son heurísticos
    de partida: recalibra con --debug --preview viendo los valores reales
    en tu cara antes de confiar en el umbral.
  - GestureController (import de openface.py) comparte la decisión: mismo modo
    INTERACCION/MOVIMIENTO, mismo requisito de volver al centro, mismo
    STOP en cuanto se suelta o se pierde la cara.
